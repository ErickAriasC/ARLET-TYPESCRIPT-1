...
let costoServicio:number=500;
// let tipoPago=prompt("¿como desea realizar su pago?\n"+"1. Moneda 2. Billete");
let tipoPago:number=2;
...
//MONEDA
function ingresarMoneda():string {
...
//BILLETE
function ingresarBillete():string {
    // let billete=prompt("Escoja el billete a ingresar\n"+"1. 2000 2. 5000");
    let billete: number=2;
    if (billete==1){
        devoluciones=2000;
        return `Has ingresado: ${devoluciones}`;
    }else if(billete==2){
...
function calcularDevoluciones():string {
